...
/**
 * Created by adahs on 22/06/2016.
 */
public class EasterEgg extends Group {
    private static final String MEDIA_URL = "http://download.oracle.com/otndocs/products/javafx/JavaRap/prog_index.m3u8";

    private MediaPlayer mediaPlayer;

    public EasterEgg(double height, double width) {
        Group root = new Group();
        mediaPlayer = new MediaPlayer(new Media(MEDIA_URL));
        mediaPlayer.setAutoPlay(true);
        PlayerPane playerPane = new PlayerPane(mediaPlayer);
        playerPane.setMinSize(height, width);
        playerPane.setPrefSize(height, width);
        playerPane.setMaxSize(height, width);
